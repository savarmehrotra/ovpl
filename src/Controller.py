...
class Controller:
    ...
    def test_lab(self, current_user, lab_id, lab_src_url, revision_tag=None):
        logger.debug("test_lab() for lab ID %s and git url %s"
                     % (lab_id, lab_src_url))
        try:
            self.lab_spec = LabManager.get_lab_reqs(lab_src_url, revision_tag)
            self.update_lab_spec(self.lab_spec, lab_id, lab_src_url,
                                 revision_tag)
            """ Commented
            if self.lab_spec['lab']['runtime_requirements']['hosting'] == \
               'dedicated':
            ...
            self.lab_vm_details = vmpoolmgr.create_vm(self.lab_spec)
            ...
            try:
                (ret_val, ret_str) = LabManager.test_lab(vmmgrurl,
                                                         port, lab_src_url,
                                                         revision_tag)
                if(ret_val):
