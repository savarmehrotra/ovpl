...
* Requirements
  ...
  3. Pre-requisites for the =AWSAdapter=:
     ...
     + The AWS key pair is placed in the appropriate location, which is same
       as what is configured in the configuration file.
...
AWS APIs to create, restart, stop and destroy VMs.
...
7. *is_running_vm* (self, vm_ip, port)

     Checks if the given TCP port at the given IP is reachable. It essentially
     checks if a particular service is running on the VM. The assumption is
     that the port is running a TCP service.
