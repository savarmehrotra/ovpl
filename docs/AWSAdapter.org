...
- Till now Ubuntu, CentOS and Debian were supported. The decision is to go
  ahead with these OSes, even though almost all the current labs(level 5 and
  hosted labs on IIIT infrastructure) use Ubuntu 10.04/12.04.
- Regarding versions of the OS, we decided:
  + Ubuntu::
    + Only LTS versions is supported.
    + Ubuntu 10.04:: Has reached end of life support we cannot support it any
      more, even though some lab developers have communicated that they have a
      hard dependency on this version of Ubuntu.
...
Hence, the current supported instance types by AWSAdapter are:
+ t2.micro (1GB RAM)
+ t2.small (2GB RAM)
...
2. The test cases are written and tested for AWSAdapter.
...
running successfully on the newly created instance on AWS.
